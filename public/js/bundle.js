...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2lrY250IiwiYSI6ImNrZHBpb2J5aDA2a3YycW15ejVtZDZ2bW4ifQ.kF35hGpULJf_THGMWlq7Hw'; // This Map function inserts the style into all the elements that have the id 'map' 

  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/sikcnt/ckdpknvmq0au51inz6gwp6rw9',
    scrollZoom: false // center: [-118.4379, 33.9772],
    // zoom: 10,
    //interactive: false

  });
  var bounds = new mapboxgl.LngLatBounds(); // After the map has been inserted into the div with the id 'map', we then add additional features to the map variable. The mapvariable injects these additional features into the map that was inserted from the code on line 8.
  ...
  locations.forEach(function (doc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(doc.coordinates).addTo(map); // Add popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(doc.coordinates).setHTML("<p>Day ".concat(doc.day, ": ").concat(doc.description, " </p>")).addTo(map); // Extend map bounds to include current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:1514/api/v1/users/updateMyPassword' : 'http://127.0.0.1:1514/api/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_51HZIGkBXHHhjpKb1PZ1aUZZZNcwv1iKMWRGUGvyveG1pfsKm5SOzTPDPNZMfgTSNUmGKzhOwrzw9ZKhYOMxMBhw1006V51mwLg');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("http://127.0.0.1:1514/api/v1/booking/checkout-session/".concat(tourID));

          case 3:
            session = _context.sent;
            console.log(session); // 2) Create checkout form and charge customer
...
if (userDataForm) userDataForm.addEventListener('submit', function (e) {
  e.preventDefault();
  var form = new FormData(); // we use this to convert the data that we get out of the dom to convert it to mulitpart form data.
